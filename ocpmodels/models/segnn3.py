...
from ocpmodels.datasets.embeddings import CONTINUOUS_EMBEDDINGS, ATOMIC_RADII
...
class SEGNN(MessagePassing):
    ...
    def __init__(self, node_in_irreps, node_hidden_irreps, node_out_irreps, attr_irreps, update_pos=False,
        ...
        irreps_message_in = (node_in_irreps + node_in_irreps + Irreps("4x0e")).simplify()
    ...
    def forward(self, x, pos, edge_index, edge_dist, edge_dist_radii_1, edge_dist_radii_2, edge_dist_radii_12, edge_attr, node_attr):
        """ Propagate messages along edges """
        x, pos = self.propagate(edge_index, x=x, pos=pos, edge_dist=edge_dist,
                                edge_dist_radii_1=edge_dist_radii_1, edge_dist_radii_2=edge_dist_radii_2, edge_dist_radii_12=edge_dist_radii_12,
                                node_attr=node_attr, edge_attr=edge_attr) # TODO: continue here!
        # x = self.feature_norm(x)
        return x, pos

    def message(self, x_i, x_j, edge_dist, edge_dist_radii_1, edge_dist_radii_2, edge_dist_radii_12, edge_attr):
        """ Message according to eqs 3-4 in the paper """
        message = self.message_layer_1(torch.cat((x_i, x_j, edge_dist, edge_dist_radii_1, edge_dist_radii_2, edge_dist_radii_12), dim=-1), edge_attr)
